...
def sample_uniform_points(boxsize = 1.01, n_points_uniform = 5000):
    ...
    points_uniform = torch.rand(n_points_uniform, 3, device=device)
...
def get_sigmas(noisy_data):
    ...
    local_sigma = torch.from_numpy(sigmas).float().to('mps')
    return local_sigma
def fast_process_data(pointcloud, n_queries = 1):
    ...
    sigmas = ptree.query(pointcloud,51,workers=10)[0][:,-1]
    ...
    n_idx = ptree.query(sample.reshape(-1,dim),1,workers=10)[1]
...
def np_train_data(point, sample, batch_size, device = torch.device("mps" if torch.backends.mps.is_available() else "cpu")):
    """
    params:
    ------
