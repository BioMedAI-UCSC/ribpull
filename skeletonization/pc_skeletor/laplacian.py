...
class LaplacianBasedContractionBase(SkeletonBase):
    ...
    def extract_topology(self):
        ...
        def manual_farthest_point_sampling(pcd, num_samples):
            ...
            for _ in range(num_samples - 1):
                # Calculate distances from all points to already selected points
                distances = []
                for i in range(n_points):
                    if i in selected_indices:
                        ...
                        for selected_idx in selected_indices:
                            dist = np.linalg.norm(points[i] - points[selected_idx])
                            min_dist = min(min_dist, dist)
                        distances.append(min_dist)
                
                # Select the point with maximum distance to nearest selected point
                next_idx = np.argmax(distances)
                selected_indices.append(next_idx)
            
            return pcd.select_by_index(selected_indices)

        self.skeleton = self.contracted_point_cloud.uniform_down_sample(every_k_points=max(1, len(self.contracted_point_cloud.points) // self.fps_points))
    ...
    def export_results(self, output: str):
        ...
        with open(path_skeleton_graph, 'wb') as f:
            pickle.dump(self.skeleton_graph, f)

        path_topology_graph = os.path.join(output, '05_topology_graph_{}'.format(self.algo_type) + '.gpickle')
        with open(path_topology_graph, 'wb') as f:
            pickle.dump(self.topology_graph, f)
